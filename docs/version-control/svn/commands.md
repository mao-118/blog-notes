...
1.  还未提交当我们想放弃对文件的修改，可以使用 svn revert 命令，将撤销任何文件或目录里的局部更改，revert 操作不单单可以使单个文件恢复原状， 而且可以使整个目录恢复原状。恢复目录需要携带 -R 参数
...
2.  已经提交对于已经提交的修改，我们需要使用 merge 命令进行回退，同时需要携带 -r 参数
